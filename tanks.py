...
def compute_prop1(elist,n,l):
    ...
    sample_atomic = lambda n: [rho_fun(rnd.normal(l_goal, delta_l)) for _ in range(n)]
...
def compute_prop2(elist,n,l):
    ...
    sample_atomic1 = lambda n: [rho_fun(rnd.normal(l_max-epsilon, epsilon)) for _ in range(n)]
    sample_atomic2 = lambda n: [rho_fun(rnd.normal(l_goal, delta_l)) for _ in range(n)]
    ...
    vlist_t1 = eval_target(n, l, sample_atomic2, rho_1, 0.3, elist)
    vlist_t2 = eval_target(n, l, sample_atomic2, rho_2, 0.3, elist)
    vlist_t3 = eval_target(n, l, sample_atomic2, rho_3, 0.3, elist)

    vlist_or = eval_or(vlist_b3,eval_or(vlist_b2,vlist_b1))
    vlist_and = eval_and(vlist_t3,eval_and(vlist_t2,vlist_t1))

    vlist_ev = eval_eventually(vlist_and,0,40)
    vlist_imp = eval_imply(vlist_or, vlist_ev)

    vlist_glob = eval_globally(vlist_imp,0,20)
...
trj = run(lambda ds: controller_q1(ds, l_goal, q_step, delta_l), q1_scenario_1(q_med,q2_dev), lambda ds: controller_q3(ds, l_goal, q_step, delta_l), ds_start, k)
plot_tanks_trajectory(k,trj,"Variation of the level of water in time (Scenario 1)","tank_level_sim_scenario_1.png")

trj = run(lambda ds: controller_q1(ds, l_goal, q_step, delta_l), q1_scenario_2(0,1), lambda ds: controller_q3(ds, l_goal, q_step, delta_l), ds_start, k)
plot_tanks_trajectory(k,trj,"Variation of the level of water in time (Scenario 2)","tank_level_sim_scenario_2.png")
#
for samples in [100]:
    elist1 = simulate(lambda ds: controller_q1(ds, l_goal, q_step, delta_l), q1_scenario_1(q_med,q2_dev),
                      lambda ds: controller_q3(ds, l_goal, q_step, delta_l), ds_start, samples, 10, k)
    plot_histogram(elist1, [100], lambda d: d['l3'], 0.0,15.0, 100, "Scenario 1: Tank 3 level probability distribution N=1000", "tank3_s1_"+str(samples)+"_")
    elist2 = simulate(lambda ds: controller_q1(ds, l_goal, q_step, delta_l), q1_scenario_2(0,1),
                      lambda ds: controller_q3(ds, l_goal, q_step, delta_l), ds_start, samples, 10, k)
    plot_histogram(elist2, [100], lambda d: d['l3'], 0.0,15.0, 100, "Scenario 2: Tank 3 level probability distribution N=1000", "tank3_s2_"+str(samples)+"_")
...
elist1 = simulate(lambda ds: controller_q1(ds, l_goal, q_step, delta_l), q1_scenario_1(q_med,q2_dev), lambda ds: controller_q3(ds, l_goal, q_step, delta_l), ds_start, n, l, k)
elist2 = simulate(lambda ds: controller_q1(ds, l_goal, q_step, delta_l), q1_scenario_2(0,1), lambda ds: controller_q3(ds, l_goal, q_step, delta_l), ds_start, n, l, k)
...
plt.title("Estimated robustness w.r.t. Prop1 (N = 100, l=10, k=150)")
...
plt.title("Estimated robustness w.r.t. Prop2 (N = 100, l=10, k=150)")
